...
SimulationEngine.prototype.stimpack_beta = function() {
  ...
  ServiceLocator.getService('getUsers').invoke(null, function(err, user_data) {
    ...
    var timer = setInterval(function() {
      ...
      var randomService = features[self.getRandomInt(0, 1)];

      //----------------
      
      console.log("User: ", targetUser, " --> Action: ", randomService.name);
      
      var params = { userId: targetUser };
      ServiceLocator.getService(randomService.name).invoke(params, function(err, follow_data) {
        console.log('follow wat');
      }, self);
